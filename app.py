...
db=mysql.connector.connect(user='root',password='Kamalesh@3112',port=3306,database='cloudsecurestoragemechanism')
cur=db.cursor()
...
@app.route('/myfiles')
def myfiles():
    sql="select id,Filename,Fileencdata1 from serverone where Receiver='%s'"%(session['receivername'])
    data1=pd.read_sql_query(sql,db)

    sql = "select id,Filename,Fileencdata1 from servertwo where Receiver='%s'" % (session['receivername'])
    data2 = pd.read_sql_query(sql, db)


    sql = "select id,Filename,Fileencdata1 from serverthree where Receiver='%s'" % (session['receivername'])
    data3 = pd.read_sql_query(sql, db)
